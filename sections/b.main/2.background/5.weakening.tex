...
The axioms in the set $g_\Omc(\alpha)$ are indeed weaker than $\alpha$ for every axiom $\alpha$. This means that, given the reference ontology $\Omc$, $\alpha$ entails every axiom $\alpha' \in g_Omc(\alpha)$, but the opposite is not necessarily true. That is, for each axiom $\alpha' \in g_\Omc(\alpha)$, $\alpha \models\Omc \alpha'$ holds.
