...
%\todo{at one point there was $R \preceq S \iff \inv(R) \preceq \inv(S)$; should it be here? it is implied: $\inv(S) \preceq \inv(S)$, $S \preceq \inv(S)$, together with $R \preceq S$ means $R \preceq \inv(S)$ means $\inv(R) \preceq \inv(S)$} and all RIAs in $\Rmc$
...
%\todok{This proof directly argues with $\preceq$...}
